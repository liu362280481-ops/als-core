...
module reaction_engine (
  ...
  always_comb begin
    mul_s_hill      = $signed({1'b0, s_cur_u}) * $signed({1'b0, hill_q8_8_u});   // Q16.16
    s_hill_q8_8     = mul_s_hill >>> 8;                                            // Q8.8
    mul_growth_gain = s_hill_q8_8 * $signed(K_REACT_Q8_8);                         // Q16.16
    p_growth_wide   = mul_growth_gain >>> 8;                                        // Q8.8

    mul_decay     = $signed({1'b0, p_cur_u}) * $signed(P_DECAY_Q8_8);              // Q16.16
    p_decay_wide  = mul_decay >>> 8;                                                // Q8.8

    s_next_wide   = $signed({1'b0, s_cur_u}) - p_growth_wide;                       // Q8.8
    p_next_wide   = $signed({1'b0, p_cur_u}) + p_growth_wide - p_decay_wide;        // Q8.8
  ...
  always_ff @(posedge aclk) begin
    if (!aresetn) begin
      ...
      if (fire_in) begin
        m_axis_tdata  <= {s_next_u, p_next_u, m_cur_u};
        m_axis_tvalid <= 1'b1;
